...
import type { BetaMarketServiceStatus } from '../../core/commercial/CommercialRadarRegistry';
...
export function RadarMetricCard({ icon, label, value, helper }: RadarMetricCardProps) {
  return (
...
export function RadarStatusBadge({ status, label }: RadarStatusBadgeProps) {
  const className =
    status === 'sellable'
...
interface RadarChecklistItemProps {
  label: string;
  done?: boolean;
}

export function RadarChecklistItem({ label, done = false }: RadarChecklistItemProps) {
  return (
