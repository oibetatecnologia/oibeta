...
export interface OpportunityCrmHandoffInput {
  responsible?: string;
  priority?: CommercialOpportunity['priority'];
  nextAction?: string;
  notes?: string;
  createTask?: boolean;
}

export interface OpportunityCrmHandoffResult {
...
export class OpportunityRepository {
  ...
  static async sendToCrm(id:string, input:OpportunityCrmHandoffInput):Promise<OpportunityCrmHandoffResult> {
    const result=await HttpRepositoryClient.post<OpportunityCrmHandoffResult>(`${API_BASE_PATH}/${encodeURIComponent(id)}/send-to-crm`,input);
    RepositoryHealthService.markApi(REPOSITORY_ID);
    return {...result, opportunity:normalizeStored(result.opportunity)};
  }
...
function normalizeStored(item:CommercialOpportunity):CommercialOpportunity {
  const normalized={...item, qualificationStatus:item.qualificationStatus||'unqualified', engagementStatus:item.engagementStatus||'new'};
